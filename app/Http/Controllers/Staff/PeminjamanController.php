...
class PeminjamanController
{
    public function index()
    {
        $peminjaman = peminjaman::select(
            ...
            ->where('peminjaman.id_pengguna', Auth::guard('staff')->user()->id_pengguna)
            ->get();
    ...
    public function store(Request $request)
    {
        ...
        peminjaman::create([
            ...
            'id_pengguna'         => Auth::guard('staff')->user()->id_pengguna,
            'id_barang'           => $request->id_barang,
            'created_pengguna_id' => Auth::guard('staff')->user()->id_pengguna,
        ]);
        return redirect()->route('staff.peminjaman.index')->with('success', 'Peminjaman berhasil diajukan, menunggu persetujuan admin.');
    }
    ...
    public function update(Request $request, $id_peminjaman)
    {
        ...
        try {
            $peminjaman->update();
            return redirect()->route('staff.peminjaman.index')->with('succeed', 'Ubah Data Peminjaman Berhasil!');
        } catch (\Exception $e) {
            return redirect()->route('staff.peminjaman.index')->with('fail', 'Ubah Data Peminjaman Gagal!');
        }
    }
    ...
    public function excel()
    {
        ...
        return Excel::download(new ExportPeminjamanPengguna(), 'Data Peminjaman-' . date(now()) . '.xlsx');
    }
    ...
    public function pdf()
    {
        $peminjaman = peminjaman::select(
            ...
            ->join('pengguna', 'pengguna.id_pengguna', '=', 'peminjaman.id_pengguna')
            ->join('barang', 'barang.id_barang', '=', 'peminjaman.id_barang')
            ->where('peminjaman.id_pengguna', Auth::guard('staff')->user()->id_pengguna) // filter sesuai user login
            ->get();
